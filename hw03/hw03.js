...
function setupMouseEvents() {
    ...
    function handleMouseUp() {
        if (isDrawing && tempEndPoint) {
            ...
            if (lines.length == 0) {
                radius = Math.sqrt((startPoint[0] - tempEndPoint[0])**2 + (startPoint[1] - tempEndPoint[1])**2);
                circleCenter = startPoint;
                lines.push(pointsToArray(getCirclePoints(startPoint, radius)));
                ...
                lines.push([...startPoint, ...tempEndPoint]);
                intersectLineCircle(startPoint, tempEndPoint, circleCenter, radius);
                if (intersectionPoints.length == 0) {
                    updateText(textOverlay3, "No intersection");
                } else if (intersectionPoints.length == 1) {
                    updateText(textOverlay3, "Intersection Point: 1 Point 1: (" + intersectionPoints[0][0].toFixed(2) + ", " + intersectionPoints[0][1].toFixed(2) + ")");
                }
                else {
...
function render() {
    ...
    for (let line of lines) {
        ...
        if (num == 0) { // 첫 번째 선분인 경우, yellow
            ...
            var num_points = NUMPOINTS
            var drawmode = gl.LINE_LOOP
        }
        ...
        gl.drawArrays(drawmode, 0, num_points);
        num++;
    }
    ...
    if (isDrawing && startPoint && tempEndPoint) {
        var num_points = 2;
        var arr = [...startPoint, ...tempEndPoint]
        var drawmode = gl.LINES
        shader.setVec4("u_color", [0.5, 0.5, 0.5, 1.0]); // 임시 선분의 color는 회색
        if (lines.length == 0) {
            var num_points = NUMPOINTS
            var drawmode = gl.LINE_LOOP
            const radius = Math.sqrt((startPoint[0] - tempEndPoint[0])**2 + (startPoint[1] - tempEndPoint[1])**2);
            arr = pointsToArray(getCirclePoints(startPoint, radius))
        }
...
async function initShader() {
    const vertexShaderSource = await readShaderFile('shader/shVert.glsl');
    const fragmentShaderSource = await readShaderFile('shader/shFrag.glsl');
...
async function main() {
    try {
        ...
        textOverlay3 = setupText(canvas, "Intersection points will be shown", 3);
...
function getCirclePoints(center, radius) {
    var points = [];
    var first = [];
    const angleStem = 2 * Math.PI / NUMPOINTS;
    for (var i = 0; i < NUMPOINTS; i++) {
        const angle = i * angleStem;
...
function intersectLineCircle(lineStart, lineEnd, circleCenter, radius) {
    const EPSILON = 1e-6; // 작은 값, 부동 소수점 비교에 사용
    const p1 = vec2.fromValues(lineStart[0], lineStart[1]);
    const p2 = vec2.fromValues(lineEnd[0], lineEnd[1]);
    const p3 = vec2.fromValues(circleCenter[0], circleCenter[1]);
    const v = vec2.create();
    vec2.sub(v, p2, p1);
    const w = vec2.create();
    vec2.sub(w, p1, p3);
    const A = vec2.dot(v, v);
    const B = 2 * vec2.dot(v, w);
    const C = vec2.dot(w, w) - radius * radius;
    ...
    if (discriminant < -EPSILON) {
        ...
        if (t >= 0 && t <= 1) {
            ...
            vec2.scaleAndAdd(intersectionPoint, p1, v, t);
            intersectionPoints.push([intersectionPoint[0], intersectionPoint[1]]);
        }
    } else {
        ...
        if (t1 >= 0 && t1 <= 1) {
            const intersectionPoint1 = vec2.create();
            vec2.scaleAndAdd(intersectionPoint1, p1, v, t1);
            intersectionPoints.push([intersectionPoint1[0], intersectionPoint1[1]]);
        }
        if (t2 >= 0 && t2 <= 1) {
            const intersectionPoint2 = vec2.create();
            vec2.scaleAndAdd(intersectionPoint2, p1, v, t2);
            intersectionPoints.push([intersectionPoint2[0], intersectionPoint2[1]]);
        }
    }
}
